...
def report_depth(sources: dict[str, Path]) -> None:
    ...
    # Median-of-ratios and ratio-of-totals answer different questions and differ by up to 3x here,
    # so both are printed with their names attached. Quoting one and describing it with the other's
    # magnitude is how a 2.6x depth difference gets written up as 8.8x.
    print(f"\n  ratios on {TRAIN_SET}, per protein (median) and summed over the set (total):")
    for label in (UNIREF_LABEL, COLABFOLD_LABEL):
        for baseline_label in (NR_LABEL, UNIREF_LABEL):
            if label == baseline_label:
                continue
            candidate = depths[label][TRAIN_SET]
            baseline = depths[baseline_label][TRAIN_SET]
            common = baseline.index.intersection(candidate.index)
            median_ratio = (candidate[common] / baseline[common].replace(0, np.nan)).median()
            total_ratio = candidate[common].sum() / baseline[common].sum()
            print(
                f"    {label:<16} / {baseline_label:<16} median {median_ratio:.2f}x   total {total_ratio:.2f}x   "
                f"deeper on {int((candidate[common] > baseline[common]).sum())} of {len(common)}"
            )
...
def report_accuracy(sources: dict[str, Path]) -> None:
    ...
    # The same means, weighted by cluster rather than by protein. This is the unit every paired
    # comparison below uses, because several proteins in the set are homologues of each other and
    # counting them separately counts the same evidence more than once. Reporting the headline in
    # one unit and the differences in another let the table disagree with the row beneath it: on
    # protein means ColabFold sits above UniRef90, on cluster means it sits below, which is what
    # its negative paired difference says.
    print(f"\n  the same {TRAIN_SET} leave-one-out means, weighted per CD-HIT cluster:")
    for label in sources:
        roc, pr = loo[label]
        roc_by_cluster = cluster_means(roc, clusters)
        pr_by_cluster = cluster_means(pr, clusters)
        print(f"    {label:<16} ROC {roc_by_cluster.mean():.3f}  PR {pr_by_cluster.mean():.3f}")
