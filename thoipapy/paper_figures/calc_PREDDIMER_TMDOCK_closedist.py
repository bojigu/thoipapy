...
def calc_closedist_from_PREDDIMER_TMDOCK_best_model(s, df_set, logging):
    ...
    for i in df_set.index:
        ...
        sys.stdout.write(f"{acc}, "), sys.stdout.flush()
        database = df_set.loc[i, "database"]
        protein = acc
        other_predictors_dir = Path(s["data_dir"]) / "Predictions/other_predictors"
        pdb_file_preddimer = os.path.join(other_predictors_dir, database, f"{protein}.preddimer.pdb")
        pdb_file_tmdock = os.path.join(other_predictors_dir, database, f"{protein}.tmdock.pdb")
        preddimer_closedist_file = os.path.join(other_predictors_dir, database, f"{protein}.preddimer.closedist.csv")
        tmdock_closedist_file = os.path.join(other_predictors_dir, database, f"{protein}.tmdock.closedist.csv")
        ...
        if os.path.isfile(pdb_file_tmdock):
            ...
            logging.warning(f"{protein} : TMDOCK pdb file not found")
            continue
        ...
        if os.path.isfile(pdb_file_preddimer):
            ...
            logging.warning(f"{protein} : PREDDIMER pdb file not found")
            continue
...
def closedist_calculate_from_dimer(acc, s, logging, pdb_file, closedist_out_csv):
    ...
    if not os.path.isfile(pdb_file):
        logging.warning(
            f"the pdb file: {pdb_file} is not existed, and skipped\n, this could be the tmdock prediction for this protein didn't exists, please try to run TMDOCK"
            " server again with different sequence TMD length input"
        )
        return None
    hash1arrayx = {}
    hash1arrayy = {}
        ...
        for line in f:
            ...
            if re.search(r"^ATOM", line):
                ...
                if atom == " OCT" and residue_name != "CEN":
                    sys.stdout.write("\n")
                    logging.warning(
                        f"{acc}  possible mislabelling of terminal residue (OCT for residue {residue_name})\nFull line:\n{line}"
                    )
                    # skip this line
                    continue
                if chain == chain1:
    ...
    for key1, _count in hash1arrayx.items():
        ...
        for key2, _count2 in hash2arrayx.items():
