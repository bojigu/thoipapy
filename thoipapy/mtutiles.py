...
def create_df_with_mean_AAIMON_each_TM(df):
    ...
    for acc in df.index:
        df.loc[acc, 'last_TM_AAIMON_ratio_mean'] = df.loc[
            acc, 'TM%02d_AAIMON_ratio_mean' % df.loc[acc, 'number_of_TMDs']]
...
def create_hist_from_df_col(df, title, axarr, row_nr, col_nr, settings, data_column, color, alpha, col_width_value,
                            fontsize, xlabel, ylabel, legend):
...
def create_line_hist_from_df_col(df, title, axarr, row_nr, col_nr, settings, data_column, color, alpha, col_width_value,
                                 fontsize, xlabel, ylabel, legend):
    # filter to remove sequences where no TMDs are found,
    df = df.loc[df['list_of_TMDs'].notnull()]
    # filter to remove sequences where no TMDs are found (if string)
...
def create_hist_from_df_col_with_auto_binlist(df, title, axarr, num_bins, row_nr, col_nr, settings, data_column, color,
                                              alpha, col_width_value, fontsize, xlabel, ylabel, legend):
...
def slice_SW_query_TMD_seq_plus_surr(x, TMD):
    return x['query_align_seq'][
           int(x['%s_start_in_SW_alignment_plus_surr' % TMD]):int(x['%s_end_in_SW_alignment_plus_surr' % TMD])]


def slice_SW_markup_TMD_plus_surr(x, TMD):
    return x['align_markup_seq'][
           int(x['%s_start_in_SW_alignment_plus_surr' % TMD]):int(x['%s_end_in_SW_alignment_plus_surr' % TMD])]


def slice_SW_match_TMD_seq_plus_surr(x, TMD):
    return x['match_align_seq'][
           int(x['%s_start_in_SW_alignment_plus_surr' % TMD]):int(x['%s_end_in_SW_alignment_plus_surr' % TMD])]
...
def score_pairwise(seq1, seq2, matrix, gap_open_penalty, gap_extension_penalty, prev_site_contained_gap=True):
    ...
    for A, B in zip(seq1, seq2):
        ...
        if gap_in_both_query_and_match:
            ...
            try:
                yield (gap_extension_penalty if prev_site_contained_gap else gap_open_penalty) if gap_exists else \
                matrix[(A, B)]
...
def save_dict_keys_as_header_in_csv(input_dict, header_fieldnames, output_csv, open_method):
    ...
    open_method_with_quotation_marks = "%s" % open_method
    with open(output_csv, open_method_with_quotation_marks) as f:
        writer = csv.DictWriter(f, fieldnames=header_fieldnames, extrasaction='ignore', delimiter=',', quotechar='"',
...
def convert_string_to_boolean_value(boolean_string):
    ...
    if str(boolean_string).lower() in ("no", "n", "false", "f", "0", "0.0", "", "none",
                                       "nein"): return False  # can also add "[]", "{}" for empty lists if desired
...
def convert_truelike_to_bool(input_item, convert_int=False, convert_float=False, convert_nontrue=True):
    ...
    """
    list_True_items = [True, 'True', "true", "TRUE", "T", "t", 'wahr', 'WAHR', 'prawdziwy', 'verdadeiro', 'sann',
                       'istinit',
                       ...
                       'VERDADERO', 'SANT', 'GWIR', 'bloody oath', 'BLOODY OATH', 'nu', 'NU', 'damn right',
                       'DAMN RIGHT']
