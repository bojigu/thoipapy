...
def gather_validation_data_for_figs(paths: ArtefactPaths, df_set, logging):
    ...
    # outputs
    validation_summary_xlsx: Path | str = paths.results_dir / "validation_summary/validation_summary.xlsx"
    ...
    for subset in subsets:
        BOCURVE_linechart_csv = (
            paths.crossvalidation_dir
            / f"compare_selected_predictors/data/{paths.setname}.{subset}.4predictors_BOCURVE_linechart.csv"
        )
        ...
        df_perc_interf_vs_p_subset = pd.read_csv(
            perc_interf_vs_PR_cutoff_linechart_single_database_data_csv, index_col=0
        )

        crossvalidation_summary_subset_xlsx: Path | str = (
            paths.results_dir / f"validation_summary/validation_summary_subset_{subset}.xlsx"
        )
...
def create_df_with_all_predictions_for_all_residues_in_set(
    paths: ArtefactPaths, df_set_nonred, pred_all_res_csv, logging
):
    ...
    for i in df_set_nonred.index:
        ...
        df_merged_new_protein["acc_db"] = f"{acc}-{database}"
        #
    ...
    if "interface_score" in df_all.columns:
        ...
        logging.warning(
            "No experimental data has been added to this dataset. Hope you're not trying to train with it!!!"
        )
