...
def homodimer_residue_closedist_calculate_from_complex(
    paths: ArtefactPaths, homodimer_structure_helix_pair_infor, logging
):
    ...
    helixpair_file_handle = open(helixpair_file)
    for row in helixpair_file_handle:
        ...
        # if pdb == "1f88":
        pdbfile = rf"/scratch2/zeng/homotypic_data/data/Bind/pdb/{pdb}.pdb"
        closedist_output_file = rf"/scratch2/zeng/homotypic_data/data/Bind/pdb/{pdb}.clodis"
        closedist_output_file_handle = open(closedist_output_file, "w")
        pdbfile_handle = open(pdbfile)
        for row1 in pdbfile_handle:
            ...
            if Atom_line:
                ...
                if (
                    chain == chain1
                    and int(chain1_pdb_tm_start) <= int(residue_num)
                    and int(residue_num) <= int(chain1_pdb_tm_end)
                ):
                ...
                if (
                    chain == chain2
                    and int(chain2_pdb_tm_start) <= int(residue_num)
                    and int(residue_num) <= int(chain2_pdb_tm_end)
                ):
                    kk = "_".join([pdb, chain, str(int(residue_num)), residue_name])
                    if atom == "CA":
                        hash2CAarrayx[kk] = x
            ...
            for key1 in hash2arrayx.keys():
                for i in range(len(arr_xvalue1)):
                    ...
                    for j in range(len(arr_xvalue2)):
                        dist = math.sqrt(
                            (float(arr_xvalue1[i]) - float(arr_xvalue2[j])) ** 2
        ...
        for key in sorted(hash1CAarrayx):
            for key1 in sorted(hash2CAarrayx):
                cadist = math.sqrt(
                    (float(hash1CAarrayx[key]) - float(hash2CAarrayx[key1])) ** 2
                    + (float(hash1CAarrayy[key]) - float(hash2CAarrayy[key1])) ** 2
                    + (float(hash1CAarrayz[key]) - float(hash2CAarrayz[key1])) ** 2
                )
        ...
        for key in sorted(hash1CBarrayx):
            for key1 in sorted(hash2CBarrayx):
                cbdist = math.sqrt(
                    (float(hash1CBarrayx[key]) - float(hash2CBarrayx[key1])) ** 2
                    + (float(hash1CBarrayy[key]) - float(hash2CBarrayy[key1])) ** 2
                    + (float(hash1CBarrayz[key]) - float(hash2CBarrayz[key1])) ** 2
                )
        ...
        for key in sorted(hashclosedist):
            ...
            try:
                if key in hashCBclosedist:
                    closedist_output_file_handle.write(
                        str(key.strip().split("_")[0])
                        + "\t"
                        + str(key.strip().split("_")[1])
                        + "\t"
                        + str(key.strip().split("_")[2])
                        + "\t"
                        + str(key.strip().split("_")[3])
                        + "\t"
                        + str(hashclosedist[key])
                        + "\t"
                        + str(hashCAclosedist[key])
                        + "\t"
                        + str(hashCBclosedist[key])
                        + "\n"
                    )
                else:
                    closedist_output_file_handle.write(
                ...
                sys.stdout.write("residue" + f"{key}" + "without CB")
