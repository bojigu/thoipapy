...
def download_homologues_from_ncbi_mult_prot(set_, df_set, logging):
    ...
    for acc in df_set.index:
        ...
        if not os.path.isfile(xml_tar_gz):
            run_download = True
            logging.info('{} starting download_homologues_from_ncbi_mult_prot'.format(acc))
        else:
        ...
        if run_download:
            download_homologues_from_ncbi(acc, query_fasta_string, blast_xml_file, xml_txt, xml_tar_gz, evalue_score, hit_list_size, logging)

    logging.info('homologues download was finished')

def download_homologues_from_ncbi(acc, query_fasta_string, blast_xml_file, xml_txt, xml_tar_gz, evalue_score, hit_list_size, logging):
    try:
        start = time.clock()
