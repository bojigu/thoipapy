...
import csv
import sys
from thoipapy.Sine_Curve.eccpy.settings import read_settings_file
from thoipapy.Sine_Curve.eccpy.judgefit import judge_fit
...
def run_curvefit(settings_excel_file):
    ...
    if True in list(dff.loc[:, "run curvefit"]):
        print("Starting run_curvefit for selected samples.\nDatafile:")
...
def calc_EC50(fn, dff, df_settings, t20):
    """ calculates the EC50 values for a single input data file
    ...
    EC50_analysis_fig_02 : Bar-chart with all calculated EC50 values for that experiment,
        full name on x-axis
    ofd_EC50_eval_csv : output summary csv file with EC50 values
        (comma separation, values in quotation marks)
    ofd_EC50_eval_excel : output summary excel file with EC50 values

    Returns
    -------
    df_eval_values : DataFrame
    ...
    amp_conc_excelfile = dff.loc[fn, "dose conc file"]
    ...
    dose_conc_excel_path = os.path.join(dff.loc[fn, "input file directory"], amp_conc_excelfile)
    ...
    dict_organising_subplots = tools.create_dict_organising_subplots(n_plots_per_fig,n_rows=nrows,n_cols=ncols)
    ...
    for sNum, sLet in enumerate(dfdose.index):
        ...
        # # normalise the datapoints between 0 and 1 to improve curve fitting
        # xnorm_orig, xmin_orig, xmax_orig = tools.normalise_0_1(x_orig)
        # ynorm_orig, ymin_orig, ymax_orig = tools.normalise_0_1(y_orig)
        ...
        #               smaller yvalue so data can be fitted to a Z-shaped sigmoidal curve)                   #
        #                                                                                                     #
        #######################################################################################################
        ...
        fig, axarr = plt.subplots(nrows=nrows, ncols=ncols, dpi=300)
        ...
        for n, d in enumerate(datasets):
            ...
            axarr[0,0].scatter(x_orig, dfe.loc["y{}".format(d), sLet], color=co[n], s=sd[d], label=d[1:], alpha=al)
        ...
        axarr[0,0].set_xlim(xmin_raw,xlim_max_plot1)
        if "_ful" in datasets:
        ...
        #######################################################################################################
            ...
            for n, d in enumerate(datasets):
                ...
                axarr[0, 0].hlines(y=y50, xmin=xmin_raw, xmax=EC50, colors = co[n], linestyles=ls[d], label='', alpha=al)
                ...
                axarr[0, 0].hlines(y=y50, xmin=xmin_raw, xmax=EC50_hill, colors = co[n], linestyles=ls[d])
            ...
            dfe = judge_fit(dfe, sLet, df_settings)
        ...
        for d in datasets:
            ...
            axarr[0,1].annotate(s=d[1:], xy=(xd[d],yaxis_pos[2]), fontsize=af, xycoords=xyc, alpha=0.75)
        ...
        for d in datasets:
            if dfe.loc["EC50_calculable{}".format(d),sLet]:
                ...
                axarr[0,1].annotate(s="%0.0f" % dfe.loc["EC50{}".format(d),sLet], xy=(xd[d],yaxis_pos[3]), fontsize=af, xycoords=xyc, alpha=0.75, color=EC50colour)
        ...
        for n,d in enumerate(datasets):
            ...
            axarr[1,1].annotate(s=d[1:], xy=(xd[d],yaxis_pos[2]), fontsize=af, xycoords=xyc, alpha=0.75)
            ...
            if EC50_calculable:
                if dfe.loc["doseconc_stepsize_at_EC50{}".format(d),"%s_okay" % sLet] == True:
                    ...
                    stepsize = dfe.loc["doseconc_stepsize_at_EC50{}".format(d),sLet]
                    ...
                    if isinstance(EC50, str):
                        ...
                        axarr[1,1].annotate(s=d[1:],xy=(xd_wide,yaxis_pos[5]), fontsize=af, xycoords=xyc, color="r")
        ...
        sys.stdout.write("\n{sLet} {s:>44} {m} : ".format(sLet=sLet, s=sample_name, m=method))
        for n, d in enumerate(datasets):
            data_seems_okay = dfe.loc["data_seems_okay{}".format(d),sLet]
            data_evaluation = "(data seems good)" if data_seems_okay else "(needs checking)"
            sys.stdout.write("{d:>4} = {EC:>5} {eval:>17}".format(d=d[1:], EC=EC50_str_dict[d], eval=data_evaluation))
            if n < len(datasets):
                sys.stdout.write(", ")
            sys.stdout.flush()
        ...
        # tempfig = os.path.join("D:\Databases\BlaTM\Lisa_Ayse\output",data_file.split(".")[0][0:10] + "_%s " % sLet + sample_name) + ".png"
        # fig.savefig(tempfig, format='png', dpi=140)
    ...
    for d in datasets:
        ...
        axarr[r].set_title('{a} data                              {b}'.format(a=d[1:],b=data_file),
        ...
        if True in list(df_eval_values.loc[:,"EC50_calculable{}".format(d)]):
            ...
            bar_container = axarr[r].bar(box_indices, y_EC50, color = t20, yerr=yerr,
                                         align="center", error_kw=dict(ecolor='k', lw=1, capsize=2, capthick=1))
        # set the xticks
        ...
        #save figure
        fig.tight_layout()
        fig.savefig(dff.loc[fn,"EC50_analysis_fig_basename"] + d + '_%02d.png'%fig_nr, format='png', dpi=150)
        fig.savefig(dff.loc[fn,"EC50_analysis_fig_basename_pdf"] + d + '_%02d.png'%fig_nr, format='pdf')
        ...
        if True in list(df_eval_values.loc[:,"EC50_calculable{}".format(d)]):
            ...
            axarr[r].set_title('{a} data                              {b}'.format(a=d[1:],b=data_file),
                                fontsize = fig_fontsize, x = 0.22)
            # create a new object (usually not used) that contains a bar-chart on figure (ax) #1
            bar_container = ax.bar(box_indices, y_EC50, color=t20, yerr=yerr,
                                   align="center", error_kw=dict(ecolor='k', lw=1, capsize=2, capthick=1))
            ...
            #save figure
            fig.tight_layout()
            fig.savefig(dff.loc[fn,"EC50_analysis_fig_basename"] + d + "_bar" + '_%02d.png'%bar_fig_nr, format='png', dpi=150)
            fig.savefig(dff.loc[fn,"EC50_analysis_fig_basename_pdf"] + d + "_bar" + '_%02d.png'%bar_fig_nr, format='pdf')
            plt.close('all')
            # tools.savefig_if_necessary(savefig, fig, bar_fig_nr, dff.loc[fn,"EC50_analysis_fig_basename"] + d + "_bar", formats = 'png', dpi=150)
            # tools.savefig_if_necessary(savefig, fig, bar_fig_nr, dff.loc[fn,"EC50_analysis_fig_basename_pdf"] + d + "_bar", formats = 'pdf')
...
def calc_EC50_brent_eq(sLet, sample_name, hill_constants, y50_norm):
    """ calculates the EC50 from a fitted curve to data normalised between 0 and 1

    Uses the brent equation (in python the brentq function) to find the position on the x-axis at a particular y-value.
    This is much more robust and accurate than the EC50 parameter in the Hill equation.
...
def standardise_doseconc_data(fn, dff, df_dose_orig, df_resp_all, data_file_path):
    ...
    ----------
        ...
        df_resp_all : DataFrame
            Dataframe with the response data, which is usually more uniform than the dose data and requires less
            standardisation. For 86-well formats, this is already created and will simply be returned. For 12-well
            formats containing dose and response data in excel format, this object will be created.
        data_file_path : path to the response data file, containing the response data for 96-well samples, and both dose and
        response data for excel input files

    Returns
    -------
        df_dose_all : DataFrame
...
def examine_input_datafile(fn, dff):
    ...
    Note
    -------
    Currently the assay_type is DIRECTLY read from the input text datafile. The assay column on the settings file
    is for personal use only. This is inflexible and will be changed in future versions, which should be able to
    allow several different excel files as inputs, with dose and response concentrations in different formats.
    """
    ...
    # obtain the identifier textstring for confirming the correct datafile
    ident_txtstr = dff.loc[fn, "textstring identifier in datafile"]
    # define the datafile
    data_file = dff.loc[fn, "response data file"]
    # define the datafile path
    data_file_path = dff.loc[fn,"data_file_path"]
    # list_96_well_doseconc_types = ["8_ampconc", "12_ampconc", "24_ampconc"]
