...
def hill_eq(hill_constants, x):
    ...
    Note that because an LD50 is the inverse of a typical EC50 graph, "top" has been renamed to "lower" and is
    generally around zero. "Bottom" has been renamed to "upper" and is generally around 1.0.

    In theory, any formula that accurately fits a sigmoidal curve is appropriate for the purpose. The Hill equation
    is preferred, because under some circumstances the EC50 is one of the fitted parameters in the hill_constants
    tuple. However this value proved to be extremely unreliable as a judge of the EC50. A more accurate EC50
    calculation is achieved through root finding using the brent equation.
...
def residuals(constants, function, x, y):
    """
    This is a value function that is used to optimise the fit of the curve to the data.
...
def normalise_0_1(arraylike):
    ...
    ----------
    arraylike : array
        Numpy array (or other arraylike) of data (float, int, etc) to be normalised.
    ...
    Usage
    ----------
    normalised_array, min_, max_ 0 normalise_0_1(original_array)
...
def reindex_df_so_selected_cols_are_first(df, selected_cols, raise_error=True):
    ...
    ----------
    df : DataFrame
        DataFrame with altered column order
    """
...
def convert_listlike_cols_to_str(df, list_cols, convert_nan=False, nanreplacement = "[]"):
    ...
    np.array(eval(stringlist))
    # convert a column of stringlists back to arrays as follows
    df.loc[:,"x_orig"] = df.loc[:,"x_orig"].apply(lambda x : np.array(eval(x)))
    """
...
# def savefig_if_necessary(savefig, fig_object, fig_nr, base_filepath, formats = 'png', dpi = 400):
#     """ Save figure to png or pdf when the subplots are all full.
#
...
#     ----------
#     savefig : bool
#         Whether the subplots are full, and the figure should be saved.
#     fig_object : matplotlib figure object
#         Figure object created by fig, ax = plt.subplots()
#     fig_nr : int
#         The figure number (i.e. canvas number)
#     base_filepath : string
#         File path, including first part of the filename.
#     formats : string, or list
#         List of formats used for saving. Default is "png". For both formats, use ["png", "pdf"].
#     dpi : int
#         Dots per inch (dpi) used in the png.
#
#     Saved Files and Figures
#     -------
#     png_figure : figure
#         Rasterised figure in png format.
#     pdf_figure : figure
#         Vector graphic figure in pdf format, compatible with Illustrator and other vector graphics programs.
#     """
#     if savefig:
#         if 'png' in formats:
#             fig_object.savefig(base_filepath + '_%02d.png' % fig_nr, format='png', dpi=dpi)
#         if 'pdf' in formats:
#             fig_object.savefig(base_filepath + '_%02d.pdf' % fig_nr, format='pdf')
...
def convert_truelike_to_bool(input_item, convert_float=False, convert_nontrue=True):
    ...
    convert_float: bool, convert floats to bool
...
def convert_nonelike_to_none(input_item):
    ...
    Note, the output is NOT the python None.
