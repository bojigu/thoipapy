...
def run_one_set(s: dict, set_number: int):
    ...
    """
    setname = "set{:02d}".format(set_number)
    ...
    if stages.run_retrieve_NCBI_homologues_with_blastp:
        thoipapy.homologues.NCBI_download.download_homologues_from_ncbi_mult_prot(paths, df_set, s["expect_value"], s["hit_list_size"], s["rerun_existing_blast_results"], logging)

    if stages.run_parse_homologues_xml_into_csv:
        thoipapy.homologues.NCBI_parser.parse_NCBI_xml_to_csv_mult_prot(paths, df_set, s["surres"], s["e_value_cutoff"], logging)

    if stages.parse_csv_homologues_to_alignment:
        thoipapy.homologues.NCBI_parser.extract_filtered_csv_homologues_to_alignments_mult_prot(paths, df_set, s["max_n_gaps_in_TMD_query_seq"], s["min_identity_of_TMD_seq"], logging)
    ...
    if stages.combine_feature_into_train_data:
        thoipapy.features.combine_features.combine_all_features_mult_prot(paths, df_set, s["lipophilicity_scale"], s["surres"], logging)
        thoipapy.features.physical_parameters.add_physical_parameters_to_features_mult_prot(paths, df_set, logging)
        thoipapy.experimental_data.add_experimental_data_to_train_set.add_experimental_data_to_combined_features_mult_prot(paths, df_set, s["inter_pair_max"], logging)
        if stages.generate_randomised_interfaces:
            thoipapy.validation.random_interface.add_random_interface_to_combined_features_mult_prot(paths, df_set, s["inter_pair_max"], logging)
        if stages.add_PREDDIMER_TMDOCK_to_combined_features:
            if True:
                thoipapy.features.preddimer_tmdock.add_PREDDIMER_TMDOCK_to_combined_features_mult_prot(paths, df_set, logging)
        if stages.remove_crystal_hetero:
            thoipapy.experimental_data.remove_hetero_contacts.remove_crystal_hetero_contact_residues_mult_prot(paths, df_set, logging)
        thoipapy.features.combine_features.combine_all_train_data_for_machine_learning(paths, df_set, stages.remove_crystal_hetero, logging)
    ...
    if stages.run_feature_selection:
        thoipapy.feature_importance.mean_decrease_impurity.get_initial_ensemble_parameters_before_feature_selection(paths, s["bind_column"], s["bootstrap"], logging)
        thoipapy.feature_importance.mean_decrease_impurity.calc_feat_import_using_MDI_before_feature_seln(paths, s["bootstrap"], logging)
        thoipapy.feature_importance.remove_duplicates.remove_duplicate_features_with_lower_MDI(paths, s["bind_column"], s["features_to_be_retained_during_selection"], s["max_similarity_duplicate_features"], logging)
        thoipapy.feature_importance.anova.select_best_features_with_anova(paths, s["bind_column"], s["n_top_features_to_keep"], logging)
        thoipapy.feature_importance.ensemble_rfe.select_best_features_with_ensemble_rfe(paths, s["bind_column"], s["n_top_features_to_keep"], s["bootstrap"], logging)
        thoipapy.feature_importance.merge.merge_top_features_anova_ensemble(paths, s["bind_column"], s["features_to_be_retained_during_selection"], logging)
    ...
    if stages.calc_feature_importances:
        thoipapy.feature_importance.mean_decrease_accuracy.calc_feat_import_from_mean_decrease_accuracy(paths, s["bind_column"], s["min_n_homol_training"], s["bootstrap"], s["n_residues_AUBOC_validation"], logging)
    ...
    if stages.conduct_ttest:
        ...
        thoipapy.experimental_data.ttest_features.conduct_ttest_for_all_features(paths, s["bind_column"], ttest_trainsetname, logging)

    if stages.train_machine_learning_model:
        thoipapy.ML_model.train_model.train_machine_learning_model(paths, s["bind_column"], s["min_n_homol_training"], s["bootstrap"], logging)

    if stages.run_testset_trainset_validation:
        thoipapy.validation.testset_trainset.run_testset_trainset_validation(paths, s["n_residues_AUBOC_validation"], *get_test_and_train_set_lists(s), logging)
    ...
    if stages.calc_PREDDIMER_TMDOCK_closedist:
        thoipapy.paper_figures.calc_PREDDIMER_TMDOCK_closedist.calc_closedist_from_PREDDIMER_TMDOCK_best_model(s, df_set, logging)
    ...
    if stages.run_validation:
        ...
        thoipapy.validation.tenfold.run_10fold_cross_validation(paths, s["min_n_homol_training"], s["cross_validation_number_of_splits"], s["bootstrap"], logging)
        thoipapy.validation.tenfold.create_10fold_cross_validation_fig(paths, s["cross_validation_number_of_splits"], logging)

        thoipapy.validation.leave_one_out.run_LOO_validation(paths, df_set, s["bind_column"], s["min_n_homol_training"], s["bootstrap"], s["n_residues_AUBOC_validation"], s["use_multiprocessing"], s["multiple_tmp_simultaneous"], logging)
        thoipapy.validation.leave_one_out.create_LOO_validation_fig(paths, df_set, s["n_residues_AUBOC_validation"], logging)
        ...
        thoipapy.validation.precision_recall.create_precision_recall_all_residues(paths, df_set, testsetname, trainsetname, logging)
    ...
    if stages.create_merged_heatmap_for_trainset_and_testset:
        thoipapy.paper_figures.create_heatmap_from_merge_file.create_merged_heatmap_for_trainset_and_testset(s, df_set, logging)

    if stages.download_10_homologues_from_ncbi:
        thoipapy.homologues.NCBI_download.download_10_homologues_from_ncbi(paths, df_set, s["rerun_existing_blast_results"], logging)
