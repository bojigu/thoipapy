...
This has since been tested much harder. The ColabFold MSA server returns 2.6x the homologues of
the 2020 archive on the median protein, and still does not improve on it: -0.017 AUC
[-0.049, +0.012] against nr, and -0.005 [-0.026, +0.016] against UniRef90. Depth was then varied
on its own -- same server, same search, same folds, differing only in a diversity filter -- and a
2.6x increase in total depth moved ROC AUC by +0.0001. The finding below, that the coevolution
features are the least reproducible part of the pipeline, replicates and is sharper there:
``DImax`` falls to rho 0.256. See ``docs/homologue_source_comparison.md``.
